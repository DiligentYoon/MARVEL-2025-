...
class Env:
    def __init__(self, episode_index, fov, sensor_range, plot=False):
        ...
        free = get_updating_node_coords(np.array([0.0, 0.0]), self.belief_info)
    ...
    def import_ground_truth(self, episode_index):
        ...
        is_free_mask = (ground_truth > 150) | ((ground_truth <= 80) & (ground_truth >= 50))
        free_indices_yx = np.argwhere(is_free_mask)
        if len(free_indices_yx) == 0:
            raise ValueError("There are no free spaces")

        start_point_index = min(10, len(free_indices_yx) - 1) 
        robot_cell_yx = free_indices_yx[start_point_index]
        robot_cell = np.array([robot_cell_yx[1], robot_cell_yx[0]])

        # 지도 값을 1 (FREE) / 255 (OCCUPIED)로 최종 변환
        final_ground_truth = np.full(is_free_mask.shape, OCCUPIED, dtype=np.uint8)
        final_ground_truth[is_free_mask] = FREE

        return final_ground_truth, robot_cell
